...
data Room = Room EncryptedName SectorId Checksum deriving Show


createRoom :: String -> Room 
createRoom str =
    let
        checksumSplitIndex = justInt . elemIndex '['
        getNameAndRest =  splitAt (lastHyphen str) str
        name = fst getNameAndRest
        rest = snd getNameAndRest
        getSectorChecksumPair = splitAt (checksumSplitIndex rest) rest
        sector = drop 1 $ fst getSectorChecksumPair
        checksum = trimEnds $ snd getSectorChecksumPair
        sectorId = read sector :: Int
    in
        Room name sectorId checksum

        
lastHyphen = head . reverse . elemIndices '-' 

trimEnds = reverse . drop 1 . reverse . drop 1
